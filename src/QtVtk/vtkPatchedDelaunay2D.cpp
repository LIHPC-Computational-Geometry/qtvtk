/*=========================================================================
  ...
  Module:    vtkPatchedDelaunay2D.cxx
...
vtkStandardNewMacro(vtkPatchedDelaunay2D);
vtkCxxSetObjectMacro(vtkPatchedDelaunay2D,Transform,vtkAbstractTransform);

// Construct object with Alpha = 0.0; Tolerance = 0.00001; Offset = 1.25;
// BoundingTriangulation turned off.
vtkPatchedDelaunay2D::vtkPatchedDelaunay2D()
	: vtkPolyDataAlgorithm ( ),										// CP
	  NumberOfDuplicatePoints (0), NumberOfDegeneracies (0),		// CP
	  Alpha (0.), Tolerance (0.00001), BoundingTriangulation (0),	// CP
	  Offset (1.), Mesh (0), Points (0)								// CP
{
  this->Alpha = 0.0;
...
//void vtkPatchedDelaunay2D::SetSourceData(vtkPolyData *input)	// CP
...
void vtkPatchedDelaunay2D::SetSourceConnection(vtkAlgorithmOutput *algOutput)
{
  this->Superclass::SetInputConnection(1, algOutput);
}

vtkPolyData *vtkPatchedDelaunay2D::GetSource()
{
...
#ifdef PATCH_VTK_MATH_SOLVE_LINEAR_SYSTEM
...
static int vtkMath5100_SolveLinearSystem(double **A, double *x, int size)
{
  ...
  if (size == 2)
	...
	// VTK 7.1.1 : est-ce cette différence qui fait qu'en VTK 7 ça part en
	// récursion infinie sur un cas bien particulier ???
	// det = vtkMath::Determinant2x2(A[0][0], A[0][1], A[1][0], A[1][1]);
...
//
// CP : Code vtkTriangle::Circumcircle de VTK 5.10.0
//
...
//                                 double center[2])
static double vtkTriangle5100_Circumcircle (
				double  x1[2], double x2[2], double x3[2], double center[2])
{
  ...
  //
//  if ( vtkMath::SolveLinearSystem(A,rhs,2) == 0 )	// CP
  if ( vtkMath5100_SolveLinearSystem (A,rhs,2) == 0 )	// CP
    {
...
#endif	// PATCH_VTK_MATH_SOLVE_LINEAR_SYSTEM
...
int vtkPatchedDelaunay2D::InCircle (double x[3], double x1[3], double x2[3],
                             double x3[3])
{
  double radius2, center[2], dist2;

#ifdef PATCH_VTK_MATH_SOLVE_LINEAR_SYSTEM
  radius2 = vtkTriangle5100_Circumcircle (x1,x2,x3,center);	// CP
#else	// PATCH_VTK_MATH_SOLVE_LINEAR_SYSTEM
	// Code d'origine. Un exemple où ça part en boucle infinie avec VTK 7.1.1
	// (mais OK avec VTK 5.10.0). Cela pourrait venir d'un test fait dans
	// vtkMath::SolveLinearSystem avec une constante différente entre VTK 5 et
	// VTK 7.
...
vtkIdType vtkPatchedDelaunay2D::FindTriangle(double x[3], vtkIdType ptIds[3],
//	vtkIdType tri, double tol, vtkIdType nei[3], vtkIdList *neighbors)	// CP
	vtkIdType tri, double tol, vtkIdType nei[3], vtkIdList *neighbors,
	vtkIdList* tried)	// CP
{
  int i, j, ir, ic, inside, i2, i3;
	int	cleanUpTried	= 0;		// CP
...
// CP :
		...
		for (vtkIdType triedId = 0; triedId < tried->GetNumberOfIds ( );
		     ++triedId)
		{
			if (newNei == tried->GetId (tried->GetNumberOfIds( ) - 1 - triedId))
				return -1;
		}	// for (vtkIdType triedId = 0; ...
// ! CP
      nei[0] = tri;
//      return this->FindTriangle(x,ptIds,newNei,tol,nei,neighbors);	// CP
      return this->FindTriangle(x,ptIds,newNei,tol,nei,neighbors, tried);// CP
      }
    }
}
...
// triangle id.
void vtkPatchedDelaunay2D::CheckEdge(vtkIdType ptId, double x[3], vtkIdType p1,
...
//
int vtkPatchedDelaunay2D::RequestData(
  ...
  vtkInformationVector *outputVector)
{
  ...
  vtkIdType *triPts = 0;
#else 	// VTK_MAJOR_VERSION < 9
	const vtkIdType *neiPts	= 0;
	const vtkIdType *triPts = 0;
#endif	// VTK_MAJOR_VERSION < 9
...
//    if ( (tri[0] = this->FindTriangle(x,pts,tri[0],tol,nei,neighbors)) >= 0 )	// CP
// Passer ici un vtkSmartPointer<vtIdList> à la place de 0 corrige une fuite
// mémoire selon valgrind 3.9.0 mais seul le premier appel fonctionne
// efficacement. Pourquoi ???
    if ( (tri[0] = this->FindTriangle(x,pts,tri[0],tol,nei,neighbors, 0)) >= 0 )	// CP
      {
...
int *vtkPatchedDelaunay2D::RecoverBoundary(vtkPolyData *source)
{
...
int vtkPatchedDelaunay2D::RecoverEdge(vtkIdType p1, vtkIdType p2)
{
  vtkIdType cellId = 0;
  int i, j, k;
...
// CP : bogue potentiel ???
// ATTENTION : j n'est pas réinitialisé et croit à la même vitesse que i
// => peut dépasser 3
// => cellId vaut n'importe quoi. Cas déjà rencontré.
// Le patch (j=0) de la v 5.2.1 semble profitable.
  for ( j=i=0; i<numRightTris; i++, j++, rightTris+=3)	// CP v 5.2.1
//  for ( i=0; i<numRightTris; i++, j++, rightTris+=3)
    {
//if (j >= 3) std::cout << __FILE__ << ' ' << __LINE__ << " ATTENTION. J = " << j << std::endl;
    cellId = tris->GetId(j);
    this->Mesh->RemoveCellReference(cellId);
    for (k=0; k<3; k++)
      {//allocate new space for cell lists
...
void vtkPatchedDelaunay2D::FillPolygons(vtkCellArray *polys, int *triUse)
{
...
//----------------------------------------------------------------------------
int vtkPatchedDelaunay2D::FillInputPortInformation(int port, vtkInformation* info)
{
  if (port == 0)
    {
...
//----------------------------------------------------------------------------
vtkAbstractTransform * vtkPatchedDelaunay2D::ComputeBestFittingPlane(
  vtkPointSet *input)
{
...
//----------------------------------------------------------------------------
void vtkPatchedDelaunay2D::PrintSelf(ostream& os, vtkIndent indent)
{
